...
<section class="content">
  <page size="A4">
      <table style="font-family:Times; font-size:12pt; margin-left:auto; margin-right:auto; line-height:17pt; width:auto; padding-top: 25;" border="0">
        ...
        <tr>
            <table border="1" id="tbutama" style="font-family:Times; font-size:8pt; width:95%; margin-left: 30;">
                <tr>
                ...
                <?php
                    $counter = 0;
                    $max = 1;
                    $query = mysqli_query($con, "SELECT kode_program, nama_program, tahun, SUM(harga) as total_program FROM view_tsc_beli where kode_program = '$program' LIMIT 0,1");
                    $query2 = mysqli_query($con, "SELECT kode_kegiatan, nama_kegiatan, SUM(harga) as total_kegiatan FROM view_tsc_beli where kode_program = '$program' LIMIT 0,1");
                    $query3 = mysqli_query($con, "SELECT kode_komponen, nama_komponen, SUM(harga) as total_komponen FROM view_tsc_beli where kode_program = '$program' GROUP BY kode_komponen");
                    while($rows = mysqli_fetch_array($query) and ($counter < $max)){
                        $th_program = $rows['tahun'];
                        $kode_program = $rows['kode_program'];
                        $nama_program = $rows['nama_program'];
                        $total_program = $rows['total_program'];
                        while($rows2 = mysqli_fetch_array($query2) and ($counter < $max)){
                            $kode_kegiatan = $rows2['kode_kegiatan'];
                            $nama_kegiatan = $rows2['nama_kegiatan'];
                            $total_kegiatan = $rows2['total_kegiatan'];
                            while($rows3 = mysqli_fetch_array($query3)){
                                ...
                                while($rows4 = mysqli_fetch_array($query4)){
                                    $kode_subkomponen = $rows4['kode_subkomponen'];
                                    $nama_subkomponen = $rows4['nama_subkomponen'];
                                    $total_subkomponen = $rows4['total_subkomponen'];                    
                ?>
                <tr>
                    <td height="20" align="left">
                        <div><span style="color:blue;"><?php echo $kode_program; ?></span></div>
                        <div><span style="color:red;">&nbsp;<?php echo $kode_kegiatan; ?></span></div>
                        <div><span style="color:black;">&nbsp;<b><?php echo $kode_kegiatan.".".$kode_komponen; ?></b></span></div>
                        <div><span style="color:black;">&nbsp;<?php echo $kode_subkomponen; ?></span></div>
                    </td>
                    <td align="left">
                        <div><span style="color:blue;"><?php echo $nama_program; ?></span><div>
                        <div><span style="color:red;"><?php echo $nama_kegiatan; ?></span><div>
                        <div><span style="color:black;">&nbsp;<b><?php echo $nama_komponen; ?></b></span></div>
                        <div><span style="color:black;">&nbsp;<?php echo $nama_subkomponen; ?></span></div>
                    </td>
                    <td align="left">&nbsp;</td>
                    <td align="left">&nbsp;</td>
                    <td align="right">
                        <div><span style="color:blue;"><?php echo rupiah($total_program); ?></span><div>
                        <div><span style="color:red;"><?php echo rupiah($total_kegiatan); ?></span><div>
                        <div><span style="color:black;">&nbsp;<b><?php echo rupiah($total_komponen); ?></b></span></div>
                        <div><span style="color:black;">&nbsp;<?php echo rupiah($total_subkomponen); ?></span></div>
                    </td>
                    <td align="left">&nbsp;</td>
                </tr>
                <?php
