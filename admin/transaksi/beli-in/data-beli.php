...
if ($_POST['truncate_data']) {
  ...
  $update_barang  = mysqli_query($con, "UPDATE ref_barang SET jumlah=0");
  $truncate_beli  = mysqli_query($con, "TRUNCATE TABLE tsc_beli");
  $truncate_pakai = mysqli_query($con, "TRUNCATE TABLE tsc_pakai");
  ...
  if ($update_barang && $truncate_beli && $truncate_pakai && $query_log) {
    echo "
      <script>
      swal({
          title: 'Sukses',
      ...
      <div class="col-md-12">
        ...
        <div class="card card-primary">
          <div class="card-header">
            <h3 class="card-title"><span class="oi oi-info"></span>&nbsp; Daftar Pembelian - BPTP Papua Barat</h3>
          </div>
          <!-- /.card-header -->
          <!-- form start -->
          <div class="card-body">
            <div class="table-responsive">
              <table id="example1" class="table table-hover table-condensed">
                ...
                <tbody>
                  <?php
                  $sql = mysqli_query($con, "SELECT * FROM view_tsc_beli GROUP BY no_dokumen") or die(mysqli_error($con));
                  $counter = 1;
                  ...
                  ?>
                    <tr>
                      ...
                      <td><?= $row['nama_agen'] ?></td>
                      <td><?= tanggal_indo($row['tgl_beli'], true) ?></td>
                      <td>
                      ...
                      <td class="text-center">
                        <a style="margin-top:2px" href="index?page=detail-beli&no_dokumen=<?php echo base64_encode($row['no_dokumen']); ?>" rel="tooltip" data-placement="top" title="Lihat Detail Data" class='btn btn-sm btn-outline-primary'><span class="oi oi-eye"></span></a>
                        <button style="margin-top:2px" onClick="confirm_delete('index?page=beli&no_dokumen=<?php echo base64_encode($row['no_dokumen']); ?>')" rel="tooltip" data-toggle="modal" data-target="#modal_delete" data-placement="top" title="Hapus Data" class='btn btn-sm btn-outline-danger'><span class="oi oi-trash"></span></button>
                      </td>
                    </tr>
                  <?php } ?>
                </tbody>
...
<div id="modal_truncate" class="modal fade">
  <div class="modal-dialog">
    <div class="modal-content bg-danger">
      ...
      <div class="modal-body">
        <p align="center">Anda akan menghapus SELURUH data <b>PEMBELIAN (IN) dan PEMAKAIAN (OUT)</b>. Peringatan! data yang telah dihapus tidak dapat dikembalikan.</p>
      </div>
      <div class="modal-footer justify-content-between">
